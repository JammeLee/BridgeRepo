...
namespace System.Net
{
	...
	public sealed class WebPermission : CodeAccessPermission, IUnrestrictedPermission
	{
		...
		public override IPermission Copy()
		{
			...
			WebPermission webPermission = new WebPermission((m_UnrestrictedConnect ? NetworkAccess.Connect : ((NetworkAccess)0)) | (m_UnrestrictedAccept ? NetworkAccess.Accept : ((NetworkAccess)0)));
		...
		public override bool IsSubsetOf(IPermission target)
		{
			...
			DelayedRegex delayedRegex = null;
			if (!webPermission.m_UnrestrictedAccept)
			{
				...
				if (m_acceptList.Count != 0)
				{
					...
					{
						delayedRegex = accept as DelayedRegex;
			...
			if (!webPermission.m_UnrestrictedConnect)
			{
				...
				if (m_connectList.Count != 0)
				{
					...
					{
						delayedRegex = connect as DelayedRegex;
						if (delayedRegex != null)
						{
		...
		private static bool isSpecialSubsetCase(string regexToCheck, ArrayList permList)
		{
			...
			{
				DelayedRegex delayedRegex = perm as DelayedRegex;
				Uri uri;
				if (delayedRegex != null)
				{
		...
		public override IPermission Union(IPermission target)
		{
			...
			else
			{
				...
				for (int i = 0; i < m_connectList.Count; i++)
				{
					DelayedRegex delayedRegex = m_connectList[i] as DelayedRegex;
					if (delayedRegex == null)
					{
			...
			else
			{
				...
				{
					DelayedRegex delayedRegex2 = m_acceptList[j] as DelayedRegex;
		...
		public override void FromXml(SecurityElement securityElement)
		{
			...
			if (securityElement2 != null)
			{
				foreach (SecurityElement child in securityElement2.Children)
				{
					if (child.Tag.Equals("URI"))
					{
						...
						AddAsPattern(NetworkAccess.Connect, new DelayedRegex(text3));
					}
				}
			}
			...
			foreach (SecurityElement child2 in securityElement2.Children)
			{
				if (child2.Tag.Equals("URI"))
				{
					...
					AddAsPattern(NetworkAccess.Accept, new DelayedRegex(text3));
				}
			}
		}
		...
		private static bool isMatchedURI(object uriToCheck, ArrayList uriPatternList)
		{
			...
			{
				DelayedRegex delayedRegex = uriPattern as DelayedRegex;
				if (delayedRegex == null)
				{
