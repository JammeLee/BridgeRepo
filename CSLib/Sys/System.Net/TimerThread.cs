...
namespace System.Net
{
	internal static class TimerThread
	{
			...
			internal bool Fire()
			{
				...
				if (flag)
				{
					...
					catch (Exception ex)
					{
						...
						if (Logging.On)
						{
							Logging.PrintError(Logging.Web, "TimerThreadTimer#" + base.StartTime.ToString(NumberFormatInfo.InvariantInfo) + "::Fire() - " + SR.GetString("net_log_exception_in_callback", ex));
						}
					}
				}
				return true;
		...
		static TimerThread()
		{
			...
			s_ThreadEvents = new WaitHandle[2]
			{
		...
		internal static Queue GetOrCreateQueue(int durationMilliseconds)
		{
			...
			TimerQueue result;
			if (weakReference == null || (result = (TimerQueue)weakReference.Target) == null)
			{
				lock (s_NewQueues)
				{
					...
					result = new TimerQueue(durationMilliseconds);
					weakReference = new WeakReference(result);
