...
namespace System.Net.Configuration
{
	internal static class ConfigurationStrings
	{
		...
		internal static string AuthenticationModulesSectionPath => GetSectionPath("authenticationModules");

		internal static string ConnectionManagementSectionPath => GetSectionPath("connectionManagement");

		internal static string DefaultProxySectionPath => GetSectionPath("defaultProxy");

		internal static string SmtpSectionPath => GetSectionPath("mailSettings", "smtp");

		internal static string RequestCachingSectionPath => GetSectionPath("requestCaching");

		internal static string SettingsSectionPath => GetSectionPath("settings");

		internal static string WebRequestModulesSectionPath => GetSectionPath("webRequestModules");

		private static string GetSectionPath(string sectionName)
		{
			return string.Format(CultureInfo.InvariantCulture, "{0}/{1}", "system.net", sectionName);
		}

		private static string GetSectionPath(string sectionName, string subSectionName)
		{
			return string.Format(CultureInfo.InvariantCulture, "{0}/{1}/{2}", "system.net", sectionName, subSectionName);
		}
	}
}
