...
namespace System.IO.Ports
{
	...
	public class SerialPort : Component
	{
		...
		private int InternalRead(char[] buffer, int offset, int count, int timeout, bool countMultiByteCharsAsOne)
		{
			...
			while (timeout == -1 || timeout - (Environment.TickCount - tickCount) > 0);
			throw new TimeoutException();
		}
		...
		public string ReadTo(string value)
		{
			...
			try
			{
				while (true)
				{
					...
					if (readTimeout == -1)
					{
						num2 = InternalRead(singleCharBuffer, 0, 1, readTimeout, countMultiByteCharsAsOne: true);
					}
					else
					{
						...
						int tickCount = Environment.TickCount;
						num2 = InternalRead(singleCharBuffer, 0, 1, readTimeout - num, countMultiByteCharsAsOne: true);
						num += Environment.TickCount - tickCount;
					}
					stringBuilder.Append(singleCharBuffer, 0, num2);
					if (c != singleCharBuffer[num2 - 1] || stringBuilder.Length < value.Length)
					{
						continue;
					}
					bool flag = true;
