...
namespace System.Collections
{
	...
	public class Hashtable : IDictionary, ICollection, IEnumerable, ISerializable, IDeserializationCallback, ICloneable
	{
		...
		private void expand()
		{
			int prime = HashHelpers.GetPrime(buckets.Length * 2);
			rehash(prime);
		}
		...
		private void Insert(object key, object nvalue, bool add)
		{
			...
			do
			{
				...
				if (buckets[num4].key == null || (buckets[num4].key == buckets && (buckets[num4].hash_coll & 0x80000000u) == 0))
				{
					if (num3 != -1)
					{
						num4 = num3;
				...
				num4 = (int)((num4 + incr) % (long)(uint)buckets.Length);
			}
		...
		{
			uint num = 1 + (((uint)hashcode >> 5) + 1) % (uint)(newBuckets.Length - 1);
		...
		public virtual void Remove(object key)
		{
			...
			do
			{
				...
				num3 = (int)((num3 + incr) % (long)(uint)buckets.Length);
			}
		...
		public virtual void GetObjectData(SerializationInfo info, StreamingContext context)
		{
			...
			{
				CompatibleComparer compatibleComparer = _keycomparer as CompatibleComparer;
		...
		public virtual void OnDeserialization(object sender)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				{
				case "LoadFactor":
					loadFactor = m_siInfo.GetSingle("LoadFactor");
					break;
				case "HashSize":
					num = m_siInfo.GetInt32("HashSize");
					break;
				case "KeyComparer":
					_keycomparer = (IEqualityComparer)m_siInfo.GetValue("KeyComparer", typeof(IEqualityComparer));
					break;
				case "Comparer":
					comparer = (IComparer)m_siInfo.GetValue("Comparer", typeof(IComparer));
					break;
				case "HashCodeProvider":
					hashCodeProvider = (IHashCodeProvider)m_siInfo.GetValue("HashCodeProvider", typeof(IHashCodeProvider));
					break;
				case "Keys":
					array = (object[])m_siInfo.GetValue("Keys", typeof(object[]));
					break;
				case "Values":
					array2 = (object[])m_siInfo.GetValue("Values", typeof(object[]));
					break;
				}
			}
			...
			version = m_siInfo.GetInt32("Version");
			m_siInfo = null;
		}
	}
}
