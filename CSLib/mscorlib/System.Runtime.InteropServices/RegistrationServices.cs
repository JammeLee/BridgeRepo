...
namespace System.Runtime.InteropServices
{
	...
	public class RegistrationServices : IRegistrationServices
	{
		...
		public virtual bool RegisterAssembly(Assembly assembly, AssemblyRegistrationFlags flags)
		{
			...
			{
				text = assembly.nGetCodeBase(fCopiedName: false);
				if (text == null)
				{
			...
			{
				RegisterPrimaryInteropAssembly(assembly, text, (PrimaryInteropAssemblyAttribute)customAttributes[j]);
			}
		...
		private bool UnregisterValueType(Type type, string strAsmVersion)
		{
			...
			if (registryKey != null)
			{
				using (RegistryKey registryKey2 = registryKey.OpenSubKey(text, writable: true))
				{
					...
					{
						registryKey2.DeleteValue("Assembly", throwOnMissingValue: false);
						registryKey2.DeleteValue("Class", throwOnMissingValue: false);
						registryKey2.DeleteValue("CodeBase", throwOnMissingValue: false);
			...
			if (flag)
			{
				if (progIdForType != string.Empty)
				{
					using (RegistryKey registryKey8 = Registry.ClassesRoot.OpenSubKey(progIdForType, writable: true))
					{
						...
						if (registryKey8.SubKeyCount != 0)
						{
							return flag;
						}
						if (registryKey8.ValueCount != 0)
						{
							return flag;
						}
			...
			while (!flag && type3 != null)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					if (!methodInfo.IsStatic)
					{
						if (bRegister)
						{
							throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_NonStaticComRegFunction"), methodInfo.Name, type3.Name));
						}
						throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_NonStaticComUnRegFunction"), methodInfo.Name, type3.Name));
					}
					...
					if (methodInfo.ReturnType != typeof(void) || parameters == null || parameters.Length != 1 || (parameters[0].ParameterType != typeof(string) && parameters[0].ParameterType != typeof(Type)))
					{
						if (bRegister)
						{
							throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_InvalidComRegFunctionSig"), methodInfo.Name, type3.Name));
						}
						throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_InvalidComUnRegFunctionSig"), methodInfo.Name, type3.Name));
					}
					if (flag)
					{
						if (bRegister)
						{
							throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_MultipleComRegFunctions"), type3.Name));
						}
						throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("InvalidOperation_MultipleComUnRegFunctions"), type3.Name));
					}
