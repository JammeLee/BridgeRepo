		...
		bool IReportMatchMembershipCondition.Check(Evidence evidence, out object usedEvidence)
		{
			...
			while (hostEnumerator.MoveNext())
			{
				...
				if (hash != null)
				{
					...
					{
						array = hash.GenerateHash(m_hashAlg);
					}
		...
		private void ParseHashValue()
		{
			lock (InternalSyncObject)
			{
				if (m_element != null)
				{
					...
					if (text == null)
					{
						throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Argument_InvalidXMLElement"), "HashValue", GetType().FullName));
					}
