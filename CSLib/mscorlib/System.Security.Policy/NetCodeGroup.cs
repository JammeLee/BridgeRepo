...
namespace System.Security.Policy
{
	...
	public sealed class NetCodeGroup : CodeGroup, IUnionSemanticCodeGroup
	{
		...
		private SecurityElement CreateWebPermission(string host, string scheme, string port)
		{
			...
			string text = TryPermissionAsOneString(array, scheme, host, intPort);
			if (text != null)
			{
			...
			else
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					text = GetPermissionAccessElementString(array[i], scheme, host, port);
		...
		{
			SecurityElement securityElement = CreateWebPermission(host, scheme, port);
		...
		private PolicyStatement CalculateAssemblyPolicy(Evidence evidence)
		{
			...
			while (hostEnumerator.MoveNext())
			{
				...
				if (url != null)
				{
					policyStatement = CalculatePolicy(url.GetURLString().Host, url.GetURLString().Scheme, url.GetURLString().Port);
				}
				else if (site == null)
				{
			...
			{
				policyStatement = CalculatePolicy(site.Name, null, null);
			}
		...
		public override bool Equals(object o)
		{
			...
			if (netCodeGroup == null || !base.Equals((object)netCodeGroup))
			{
				return false;
			}
