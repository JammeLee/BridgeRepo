...
namespace System.Runtime.Serialization.Formatters.Binary
{
	internal sealed class ObjectWriter
	{
		...
		internal void Serialize(object graph, Header[] inHeaders, __BinaryWriter serWriter, bool fCheck)
		{
			...
			{
				flag = true;
				graph = WriteMethodCall(methodCallMessage);
			}
			else
			{
				...
				{
					flag2 = true;
					graph = WriteMethodReturn(methodReturnMessage);
				}
			}
			if (graph == null)
			{
			...
			while ((next = GetNext(out objID)) != null)
			{
				...
				{
					writeObjectInfo = WriteObjectInfo.Serialize(next, m_surrogates, m_context, serObjectInfoInit, m_formatterConverter, this);
		...
		private void Write(WriteObjectInfo objectInfo, NameInfo memberNameInfo, NameInfo typeNameInfo)
		{
			...
			{
				Type type = ((outMemberTypes[i] != null) ? outMemberTypes[i] : ((outMemberData[i] == null) ? Converter.typeofObject : GetType(outMemberData[i])));
				if (ToCode(type) == InternalPrimitiveTypeE.Invalid && type != Converter.typeofString)
				{
					if (outMemberData[i] != null)
					{
						array[i] = WriteObjectInfo.Serialize(outMemberData[i], m_surrogates, m_context, serObjectInfoInit, m_formatterConverter, this);
						array[i].assemId = GetAssemblyId(array[i]);
					}
					else
					{
						array[i] = WriteObjectInfo.Serialize(outMemberTypes[i], m_surrogates, m_context, serObjectInfoInit, m_formatterConverter);
						array[i].assemId = GetAssemblyId(array[i]);
					}
				}
			}
		...
		private void WriteMembers(NameInfo memberNameInfo, NameInfo memberTypeNameInfo, object memberData, WriteObjectInfo objectInfo, NameInfo typeNameInfo, WriteObjectInfo memberObjectInfo)
		{
			...
			if (nItype == Converter.typeofObject || Nullable.GetUnderlyingType(nItype) != null)
			{
		...
		private void WriteArray(WriteObjectInfo objectInfo, NameInfo memberNameInfo, WriteObjectInfo memberObjectInfo)
		{
			...
			{
				writeObjectInfo = WriteObjectInfo.Serialize(elementType, m_surrogates, m_context, serObjectInfoInit, m_formatterConverter);
				writeObjectInfo.assemId = GetAssemblyId(writeObjectInfo);
			}
			NameInfo nameInfo = null;
		...
		private void WriteArrayMember(WriteObjectInfo objectInfo, NameInfo arrayElemTypeNameInfo, object data)
		{
			...
			if (!WriteKnownValueClass(arrayElemTypeNameInfo, nameInfo, data))
			{
				...
				if (num < 1)
				{
					WriteObjectInfo writeObjectInfo = WriteObjectInfo.Serialize(data, m_surrogates, m_context, serObjectInfoInit, m_formatterConverter, this);
		...
		private object GetNext(out long objID)
		{
			...
			if (firstTime)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Serialization_ObjNoID"), obj2));
			}
			return obj;
		}
		...
		private long GetAssemblyId(WriteObjectInfo objectInfo)
		{
			...
			else
			{
				...
				serWriter.WriteAssembly(objectInfo.GetTypeFullName(), objectInfo.objectType, assemblyString2, (int)num, isNew, isInteropType: false);
			}
			return num;
		}
