	...
	internal class EventProviderWriter
	{
		...
		public Type Perform()
		{
			...
			for (int i = 0; i < nonPropertyMethods.Length; i++)
			{
				...
				{
					DefineAddEventMethod(typeBuilder, nonPropertyMethods[i], m_SinkHelperType, fieldBuilder, fbEventCP, mbInitSrcItf);
					DefineRemoveEventMethod(typeBuilder, nonPropertyMethods[i], m_SinkHelperType, fieldBuilder, fbEventCP);
				}
			}
		...
		private MethodBuilder DefineAddEventMethod(TypeBuilder OutputTypeBuilder, MethodInfo SrcItfMethod, Type SinkHelperClass, FieldBuilder fbSinkHelperArray, FieldBuilder fbEventCP, MethodBuilder mbInitSrcItf)
		{
			...
			MethodBuilder methodBuilder = OutputTypeBuilder.DefineMethod(parameterTypes: new Type[1]
			{
				field.FieldType
			}, name: "add_" + SrcItfMethod.Name, attributes: MethodAttributes.Public | MethodAttributes.Virtual, returnType: null);
		...
		private MethodBuilder DefineRemoveEventMethod(TypeBuilder OutputTypeBuilder, MethodInfo SrcItfMethod, Type SinkHelperClass, FieldBuilder fbSinkHelperArray, FieldBuilder fbEventCP)
		{
			...
			{
				field.FieldType
			}, name: "remove_" + SrcItfMethod.Name, attributes: MethodAttributes.Public | MethodAttributes.Virtual, returnType: null);
			ILGenerator iLGenerator = methodBuilder.GetILGenerator();
		...
		private void DefineConstructor(TypeBuilder OutputTypeBuilder, FieldBuilder fbCPC)
		{
			...
			MethodBuilder methodBuilder = OutputTypeBuilder.DefineMethod(".ctor", attributes, null, new Type[1]
			{
				typeof(object)
			});
			ILGenerator iLGenerator = methodBuilder.GetILGenerator();
