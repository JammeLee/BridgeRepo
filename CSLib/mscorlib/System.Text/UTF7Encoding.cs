...
namespace System.Text
{
	...
	public class UTF7Encoding : Encoding
	{
		...
		internal sealed class DecoderUTF7Fallback : DecoderFallback
		{
			...
			{
				DecoderUTF7Fallback decoderUTF7Fallback = value as DecoderUTF7Fallback;
				if (decoderUTF7Fallback != null)
				{
					return true;
				}
				return false;
		...
		private void OnDeserialized(StreamingContext ctx)
		{
			...
			{
				m_allowOptionals = directEncode["!\"#$%&*;<=>@[]^_`{|}"[0]];
			}
			MakeTables();
		}
		...
		internal unsafe override int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, EncoderNLS baseEncoder)
		{
			...
			while (encodingByteBuffer.MoreData)
			{
				...
				if (nextChar < '\u0080' && directEncode[nextChar])
				{
					if (num2 >= 0)
					{
						if (num2 > 0)
