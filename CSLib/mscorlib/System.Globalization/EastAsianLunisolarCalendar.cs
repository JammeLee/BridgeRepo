...
namespace System.Globalization
{
	...
	public abstract class EastAsianLunisolarCalendar : Calendar
	{
		...
		public override DateTime ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int era)
		{
			...
			if (day < 1 || day > num)
			{
				throw new ArgumentOutOfRangeException("day", string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("ArgumentOutOfRange_Day"), num, month));
			}
			int nSolarYear = 0;
			int nSolarMonth = 0;
			int nSolarDay = 0;
		...
		internal void GregorianToLunar(int nSYear, int nSMonth, int nSDate, ref int nLYear, ref int nLMonth, ref int nLDate)
		{
			...
			{
				nLYear--;
				num3 += ((GergIsleap(nLYear) == 1) ? 366 : 365);
