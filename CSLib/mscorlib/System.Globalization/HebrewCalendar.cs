...
namespace System.Globalization
{
	...
	public class HebrewCalendar : Calendar
	{
		...
		private static readonly int[] m_HebrewTable = new int[1316]
		{
			7,
			3,
			17,
		...
		private static readonly int[,] m_lunarMonthLen = new int[7, 14]
		{
			{
				0,
				0,
		...
		internal int GetLunarMonthDay(int gregorianYear, __DateBuffer lunarDate)
		{
			...
			num *= 2;
			lunarDate.day = m_HebrewTable[num];
			int result = m_HebrewTable[num + 1];
		...
		internal virtual int GetDatePart(long ticks, int part)
		{
			...
			if (num2 + _DateBuffer.day <= m_lunarMonthLen[num, _DateBuffer.month])
			{
			...
			_DateBuffer2.month++;
			_DateBuffer2.day = 1;
			num2 -= m_lunarMonthLen[num, _DateBuffer.month] - _DateBuffer.day;
			if (num2 > 1)
			{
				while (num2 > m_lunarMonthLen[num, _DateBuffer2.month])
				{
					num2 -= m_lunarMonthLen[num, _DateBuffer2.month++];
					if (_DateBuffer2.month > 13 || m_lunarMonthLen[num, _DateBuffer2.month] == 0)
					{
						_DateBuffer2.year++;
						num = m_HebrewTable[(year + 1 - 1583) * 2 + 1];
		...
		internal int GetHebrewYearType(int year, int era)
		{
			...
			return m_HebrewTable[(year - 3760 - 1583) * 2 + 1];
		}

		public override int GetDayOfYear(DateTime time)
		{
			int year = GetYear(time);
			DateTime dateTime = ToDateTime(year, 1, 1, 0, 0, 0, 0, 0);
		...
		public override int GetDaysInMonth(int year, int month, int era)
		{
			...
			int num = m_lunarMonthLen[hebrewYearType, month];
			if (num == 0)
			{
