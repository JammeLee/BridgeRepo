...
namespace System.Security.Principal
{
	...
	public sealed class SecurityIdentifier : IdentityReference, IComparable<SecurityIdentifier>
	{
		...
		private void CreateFromParts(IdentifierAuthority identifierAuthority, int[] subAuthorities)
		{
			...
			for (byte b = 0; b < SubAuthorityCount; b = (byte)(b + 1))
			{
		...
		private void CreateFromBinaryForm(byte[] binaryForm, int offset)
		{
			...
			for (byte b = 0; b < binaryForm[offset + 1]; b = (byte)(b + 1))
			{
		...
		private static IdentityReferenceCollection TranslateToNTAccounts(IdentityReferenceCollection sourceSids, out bool someFailed)
		{
			...
			try
			{
				foreach (IdentityReference sourceSid in sourceSids)
				{
					...
					ref GCHandle reference = ref array2[num];
					reference = GCHandle.Alloc(securityIdentifier.BinaryForm, GCHandleType.Pinned);
					ref IntPtr reference2 = ref array[num];
					reference2 = array2[num].AddrOfPinnedObject();
					num++;
				}
				...
				if (num2 == 0 || num2 == 263)
				{
					...
					{
						Win32Native.LSA_TRUST_INFORMATION lSA_TRUST_INFORMATION = (Win32Native.LSA_TRUST_INFORMATION)Marshal.PtrToStructure(new IntPtr((long)lSA_REFERENCED_DOMAIN_LIST.Domains + num * Marshal.SizeOf(typeof(Win32Native.LSA_TRUST_INFORMATION))), typeof(Win32Native.LSA_TRUST_INFORMATION));
						array3[num] = Marshal.PtrToStringUni(lSA_TRUST_INFORMATION.Name.Buffer, (int)lSA_TRUST_INFORMATION.Name.Length / 2);
					}
					for (num = 0; num < sourceSids.Count; num++)
					{
						Win32Native.LSA_TRANSLATED_NAME lSA_TRANSLATED_NAME = (Win32Native.LSA_TRANSLATED_NAME)Marshal.PtrToStructure(new IntPtr((long)names.DangerousGetHandle() + num * Marshal.SizeOf(typeof(Win32Native.LSA_TRANSLATED_NAME))), typeof(Win32Native.LSA_TRANSLATED_NAME));
						...
						default:
							someFailed = true;
							identityReferenceCollection.Add(sourceSids[num]);
							break;
						}
					}
				}
