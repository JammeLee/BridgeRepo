...
namespace System.Security.AccessControl
{
	public sealed class RawAcl : GenericAcl
	{
		...
		public override int BinaryLength => _binaryLength;
		...
		internal void SetBinaryForm(byte[] binaryForm, int offset)
		{
			...
			while (true)
			{
				if (num < count)
				{
					...
					if (_binaryLength + binaryLength > GenericAcl.MaxBinaryLength)
					{
		...
		public void InsertAce(int index, GenericAce ace)
		{
			...
			if (_binaryLength + ace.BinaryLength > GenericAcl.MaxBinaryLength)
			{
				throw new OverflowException(Environment.GetResourceString("AccessControl_AclTooLong"));
			}
		...
		public void RemoveAce(int index)
		{
			...
			_binaryLength -= genericAce.BinaryLength;
		}
	}
}
