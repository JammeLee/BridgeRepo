...
namespace System.Security
{
	...
	public class PermissionSet : ISecurityEncodable, ICollection, IEnumerable, IStackWalk, IDeserializationCallback
	{
		...
		{
			if (((uint)context.State & 0xFFFFFF3Fu) != 0)
			{
				m_serializedPermissionSet = null;
		...
		internal IPermission GetPermission(int index)
		{
			...
			permission = CreatePermission(item, index);
			if (permission == null)
			{
				return null;
			}
		...
		{
			return SetPermission(perm, fReplace: true);
		}
		...
		public IPermission AddPermission(IPermission perm)
		{
			...
			if (IsUnrestricted() && (token.m_type & PermissionTokenType.IUnrestricted) != 0)
			{
				...
				return (IPermission)Activator.CreateInstance(type, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public, null, new object[1]
				{
		...
		internal bool CheckDeny(PermissionSet deniedSet, out IPermission firstPermThatFailed)
		{
			...
			while (permissionSetEnumeratorInternal.MoveNext())
			{
				CodeAccessPermission codeAccessPermission = permissionSetEnumeratorInternal.Current as CodeAccessPermission;
		...
		internal void InplaceUnion(PermissionSet other)
		{
			...
			if (m_Unrestricted)
			{
				...
				if (m_canUnrestrictedOverride && other.m_canUnrestrictedOverride)
				{
					m_permSet = null;
					return;
				}
		...
		public PermissionSet Union(PermissionSet other)
		{
			...
			if (permissionSet.m_Unrestricted)
			{
				...
				if (other.m_canUnrestrictedOverride)
				{
					permissionSet.m_permSet = ((m_permSet != null) ? new TokenBasedSet(m_permSet) : null);
					return permissionSet;
				}
			}
			CheckSet();
		...
		private void NormalizePermissionSet()
		{
			...
			if (m_permSet != null)
			{
				for (int i = m_permSet.GetStartingIndex(); i <= m_permSet.GetMaxUsedIndex(); i++)
				{
					...
					if (securityElementFactory != null)
					{
						permission = CreatePerm(securityElementFactory);
					}
					if (permission != null)
					{
		...
		private bool DecodeUsingSerialization(byte[] data)
		{
			...
			if (obj != null)
			{
				permissionSet = (PermissionSet)obj;
				m_Unrestricted = permissionSet.m_Unrestricted;
				m_permSet = permissionSet.m_permSet;
				m_CheckedForNonCas = false;
				return true;
			}
		...
		internal virtual void FromXml(SecurityElement et, bool allowInternalOnly, bool ignoreTypeLoadFailures)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (permissionToken != null && obj != null)
				{
					...
					if (m_Unrestricted && obj is IPermission && CodeAccessPermission.CanUnrestrictedOverride((IPermission)obj))
					{
						obj = null;
					}
		...
		internal IPermission CreatePermission(object obj, int index)
		{
			...
			if (m_Unrestricted && CodeAccessPermission.CanUnrestrictedOverride(permission))
			{
				permission = null;
			}
			CheckSet();
		...
		internal SecurityElement InternalToXml()
		{
			...
			if (m_permSet != null)
			{
				...
				for (int i = m_permSet.GetStartingIndex(); i <= maxUsedIndex; i++)
				{
					...
					{
						if (!m_Unrestricted || !CodeAccessPermission.CanUnrestrictedOverride((IPermission)item))
						{
