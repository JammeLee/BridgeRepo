	...
	internal sealed class SecurityDocument
	{
		...
		public void AddString(string str, ref int position)
		{
			...
			for (int i = 0; i < str.Length; i++)
			{
				...
				m_data[position + 2 * i + 1] = (byte)(str[i] & 0xFFu);
			}
		...
		internal SecurityElement InternalGetElement(ref int position, bool bCreate)
		{
			...
			string @string = GetString(ref position, bCreate);
			if (bCreate)
			{
			...
			{
				position++;
				string string2 = GetString(ref position, bCreate);
				string string3 = GetString(ref position, bCreate);
				if (bCreate)
				{
			...
			{
				position++;
				string string4 = GetString(ref position, bCreate);
				if (bCreate)
				{
		...
		public string GetAttributeForElement(int position, string attributeName)
		{
			...
			{
				position++;
				string @string = GetString(ref position);
				string string2 = GetString(ref position);
