...
namespace System.Reflection.Emit
{
	internal class ScopeTree
	{
		...
		internal int GetCurrentActiveScopeIndex()
		{
			...
			{
				num = ((m_ScopeActions[num2] != 0) ? (num + 1) : (num - 1));
				num2--;
			}
			return num2;
		}
		...
		internal void EnsureCapacity()
		{
			...
			{
				int[] array = new int[m_iCount * 2];
				Array.Copy(m_iOffsets, array, m_iCount);
				m_iOffsets = array;
				ScopeAction[] array2 = new ScopeAction[m_iCount * 2];
				Array.Copy(m_ScopeActions, array2, m_iCount);
				m_ScopeActions = array2;
				LocalSymInfo[] array3 = new LocalSymInfo[m_iCount * 2];
