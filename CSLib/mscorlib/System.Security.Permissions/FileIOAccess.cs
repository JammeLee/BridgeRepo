...
namespace System.Security.Permissions
{
	...
	internal sealed class FileIOAccess
	{
		...
		public FileIOAccess Intersect(FileIOAccess operand)
		{
			...
			if (m_allLocalFiles)
			{
				...
				if (array != null)
				{
					for (int i = 0; i < array.Length; i++)
					{
						...
						{
							stringExpressionSet.AddExpressions(new string[1]
							{
								array[i]
							}, checkForDuplicates: true, needFullPath: false);
						}
					}
				}
			}
			...
			{
				string[] array2 = m_set.ToStringArray();
				if (array2 != null)
				{
					for (int j = 0; j < array2.Length; j++)
					{
						string root2 = GetRoot(array2[j]);
						if (root2 != null && _LocalDrive(GetRoot(root2)))
						{
							stringExpressionSet.AddExpressions(new string[1]
							{
								array2[j]
			...
			string[] array3 = m_set.Intersect(operand.m_set).ToStringArray();
			if (array3 != null)
			{
		...
		public bool IsSubsetOf(FileIOAccess operand)
		{
			...
			if ((!m_pathDiscovery || !m_set.IsSubsetOfPathDiscovery(operand.m_set)) && !m_set.IsSubsetOf(operand.m_set))
			{
				...
				string[] array = m_set.ToStringArray();
				for (int i = 0; i < array.Length; i++)
				{
					string root = GetRoot(array[i]);
					if (root == null || !_LocalDrive(GetRoot(root)))
