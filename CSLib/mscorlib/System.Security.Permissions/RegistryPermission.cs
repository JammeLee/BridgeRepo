	...
	public sealed class RegistryPermission : CodeAccessPermission, IUnrestrictedPermission, IBuiltInPermission
	{
		...
		public override SecurityElement ToXml()
		{
			...
			if (!IsUnrestricted())
			{
				...
				{
					securityElement.AddAttribute("Create", SecurityElement.Escape(m_create.ToString()));
				}
				if (m_viewAcl != null && !m_viewAcl.IsEmpty())
				{
					securityElement.AddAttribute("ViewAccessControl", SecurityElement.Escape(m_viewAcl.ToString()));
				}
				if (m_changeAcl != null && !m_changeAcl.IsEmpty())
				{
					securityElement.AddAttribute("ChangeAccessControl", SecurityElement.Escape(m_changeAcl.ToString()));
				}
			}
			else
			{
