...
namespace System.Security.Permissions
{
	...
	public sealed class IsolatedStorageFilePermission : IsolatedStoragePermission, IBuiltInPermission
	{
		...
		public override bool IsSubsetOf(IPermission target)
		{
			if (target == null)
			{
				if (m_userQuota == 0 && m_machineQuota == 0 && m_expirationDays == 0 && !m_permanentData)
				{
		...
		public override IPermission Intersect(IPermission target)
		{
			...
			if (isolatedStorageFilePermission2.m_userQuota == 0 && isolatedStorageFilePermission2.m_machineQuota == 0 && isolatedStorageFilePermission2.m_expirationDays == 0 && !isolatedStorageFilePermission2.m_permanentData && isolatedStorageFilePermission2.m_allowed == IsolatedStorageContainment.None)
			{
				return null;
			}
			return isolatedStorageFilePermission2;
